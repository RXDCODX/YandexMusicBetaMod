...
namespace YandexMusicPatcherGui
{
    public static class Patcher
    {
        ...
        public static bool IsModInstalled()
        {
            ...
            var filesToCheck = new string[] { "Яндекс Музыка.exe", "resources/app.asar" }.Select(
                x => Path.GetFullPath(Path.Combine(Program.ModPath, x))
            );
        ...
        public static async Task InstallMods(string appPath)
        {
            ...
            var newModsPath = Path.GetFullPath(
                Path.Combine(appPath, "app/_next/static/yandex_mod")
            );
            ...
            ReplaceFileContents(
                ...
                "modConfig = "
                    + JsonConvert.SerializeObject(
                        Program.Config.Mods.ToDictionary(x => x.Tag, x => x.Enabled),
                        Formatting.Indented
                    )
            );
            ...
            if (Program.Config.HasMod("discordRPC"))
            {
                ...
                processStartInfo.Arguments =
                    $"x \"{Path.GetFullPath("mods/DiscordRPC/Lib/node_modules.zip")}\" -o{Path.Combine(appPath, "node_modules")} -y";
            ...
            File.WriteAllText(
                Path.Combine(appPath, "main/lib/preload.js"),
                File.ReadAllText(Path.Combine(appPath, "main/lib/preload.js"))
            ...
            File.WriteAllText(
                Path.Combine(appPath, "main/lib/systemMenu.js"),
            ...
            File.WriteAllText(
                Path.Combine(appPath, "main/config.js"),
            ...
            if (Program.Config.HasMod("useDevTools"))
                File.WriteAllText(
                    Path.Combine(appPath, "main/lib/createWindow.js"),
        ...
        public static async Task DownloadLastestMusic()
        {
            ...
            processStartInfo.Arguments =
                $"x \"{Path.GetFullPath("temp/stable.exe")}\" -o{Program.ModPath} -y";
            processStartInfo.RedirectStandardInput = true;
            processStartInfo.RedirectStandardOutput = true;
            processStartInfo.WindowStyle = ProcessWindowStyle.Hidden;
            processStartInfo.UseShellExecute = false;
